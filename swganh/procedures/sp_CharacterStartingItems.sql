...
BEGIN
	...
	DECLARE tmpId BIGINT(20);
        ...
        DECLARE cur_1 CURSOR FOR SELECT family_id,type_id,customName FROM swganh_static.starting_items WHERE req_id = (SELECT id FROM starting_item_requirements WHERE species=race_id AND gender=gender_id AND profession=profession_id);
        ...
        REPEAT
                ...
                IF NOT loopEnd THEN
						IF tutorialcontainer_id = 0 THEN
							...
							IF familyId = 7 THEN
									SELECT swganh.sf_DefaultItemCreate(familyId,typeId,inventory_id-1,0,99,0,0,0,custom_name) INTO tmpId;

									UPDATE swganh.item_attributes SET value='1' WHERE item_id=tmpId AND attribute_id=11;
							ELSE
									SELECT swganh.sf_DefaultItemCreate(familyId,typeId,inventory_id,0,99,0,0,0,custom_name) INTO tmpId;
						...
						ELSE
							...
							ELSE
								...
								IF familyId <> 10 OR typeId <> 2326 THEN
									SELECT swganh.sf_DefaultItemCreate(familyId,typeId,tutorialcontainer_id,privateowner_id,99,0,0,0,custom_name) INTO tmpId;
								END IF;
